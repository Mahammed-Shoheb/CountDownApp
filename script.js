...
const displayEvents = (eventName, endDate) => {
  const EvtNode = document.createElement("article");
  ...
  const eventsCollection = {
    ...
    display() {
      this.intervalID = setInterval(() => {
        ...
        this.ele.innerHTML = `<h2 class="eventName title">${eventName.toUpperCase()}</h2>
...
removeEventForm.addEventListener("submit", (e) => {
  // e.preventDefault();
  const eventName = document
    .getElementById("eventNameRemove")
    .value.trim()
    .toLowerCase();
  // console.log(eventName);
